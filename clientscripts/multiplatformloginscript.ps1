...
function checkssmservice {
    ...
    write-log -Message " Checking SSM service Status " -path $logfile -level INFO
    switch ($BaseOS) {
        linux {
            $serivicestatus = systemctl show -p SubState amazon-ssm-agent | cut -d “=” -f2
            write-log -Message " SSM service status is $serivicestatus " -path $logfile -level INFO
        }
        Windows {
...
function installssm {
    ...
    $wstagstringurladd = "wsmiaddtag/?wsid=" + $workspaceID + "&miid=" + $managedinstid.'instance-id'+ "&hostname=" + $hostname + "&username=" + $username + "&directoryid=" + $DirectoryID + "&wsregion=" + $wsregion +  "&OS=" +$BaseOS
...
if (!$domainname) { 
    ...
    if (!$username) {
        ...
        if ( $servicesstatus -eq "running") {
            ...
            write-log -Message "SSM service is in a unknow state trying to recycle SSM service" -path $logfile -level ERROR
            switch ($BaseOS) {
                Linux
                { systemctl stop amazon-ssm-agent }
                Windows
                { Get-process Amazon-ssm-agent | stop-process -Force
                    Get-Process  }
            }
            ssmclean($BaseOS)
            installssm $username
